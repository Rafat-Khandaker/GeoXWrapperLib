...
namespace GeoXWrapperTest.Model.Display
{
    public class FBNDisplay
    {
        ...
        public string in_bin => _wa1.in_bin.BINToString();
        ...
        public string out_bbl_boro => _wa2f1ax.bbl.boro;
        ...
        public string out_vacant_flag => string.IsNullOrWhiteSpace(_wa2f1ax.vacant_flag)
            ? "Not Vacant"
            : _wa2f1ax.vacant_flag;
        public string out_condo_flag
        {
            get
            {
                switch (_wa2f1ax.condo_flag)
                {
                    case " ":
                        return "Non-Condo";
                    case "C":
        ...
        public string out_num_of_bldgs => string.IsNullOrEmpty(_wa2f1ax.num_of_bldgs.Trim()) || string.IsNullOrEmpty(_wa2f1ax.num_of_bldgs.TrimStart('0'))
            ? "0"
        ...
        public string out_irreg_flag => string.IsNullOrWhiteSpace(_wa2f1ax.irreg_flag)
            ? "Not Irregular Lot"
            : _wa2f1ax.irreg_flag;
        public string out_condo_num => string.IsNullOrWhiteSpace(_wa2f1ax.condo_num) || string.Equals("0", _wa2f1ax.condo_num)
            ? "N/A"
            : _wa2f1ax.condo_num;
        public string out_coop_num => string.IsNullOrWhiteSpace(_wa2f1ax.coop_num) || string.Equals("0000", _wa2f1ax.coop_num)
            ? "N/A"
            : _wa2f1ax.coop_num;
        ...
        public string out_bin_status => string.Equals(_wa1.in_tpad_switch, "Y", StringComparison.OrdinalIgnoreCase)
            ? _fld.get_get_short_def("TPAD_bin_status", _wa2f1ax.TPAD_bin_status)
            : "N/A";
        public string out_TPAD_bin => string.Equals(_wa1.in_tpad_switch, "Y", StringComparison.OrdinalIgnoreCase)
            ? _fld.get_get_short_def("TPAD_bin_status", _wa2f1ax.TPAD_new_bin.ToString())
            : "N/A";
        public string out_TPAD_bin_status => string.Equals(_wa1.in_tpad_switch, "Y", StringComparison.OrdinalIgnoreCase)
            ? _fld.get_get_short_def("TPAD_bin_status", _wa2f1ax.TPAD_new_bin_status)
            : "N/A";
        public string out_TPAD_conflict_flag => string.Equals(_wa1.in_tpad_switch, "Y", StringComparison.OrdinalIgnoreCase)
            ? _wa2f1ax.TPAD_conflict_flag
            : "N/A";
        public string out_x_coord_property => _wa2f1ax.x_coord.TrimStart('0');
        public string out_y_coord_property => _wa2f1ax.y_coord;
    }
}
